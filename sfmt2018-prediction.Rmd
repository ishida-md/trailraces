...
both_clock <- both_stat %>% mutate_at(vars(banff:finish), '+', 19.5 * 3600 + 15 * 3600 * 24) %>% 
  mutate_at(vars(banff:finish), seconds_to_period) %>% 
  mutate_at(vars(banff:finish), str_extract, ".+M")

both_clock
```

```{r eval = F}
write_csv(both_clock, "output/sfmt-100m-pred.csv")
```
